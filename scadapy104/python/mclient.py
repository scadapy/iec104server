...
def Proc_7(f=1):
     ...
     for i in range(0,unitCount+1):
         ...
         timeOut.append(i)
     try:
         com=serial.Serial('1',1, timeout=2, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS)
         time.sleep(2) 
     except Exception as e:
          pass
     x=0; varname[x],rtuAddress[x],reg[x],startAdr[x],rangeAdr[x],timeOut[x]=('t','1','','','1','1')
     while True:
